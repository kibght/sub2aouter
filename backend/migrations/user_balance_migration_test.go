...
func TestMigration192EnforcesNonNegativeUserBalance(t *testing.T) {
	...
	require.Contains(t, sql, "IF TG_OP = 'INSERT' THEN")
	require.Contains(t, sql, "ELSIF NEW.balance IS DISTINCT FROM OLD.balance THEN")
	require.NotContains(t, sql, "IF TG_OP = 'INSERT' OR NEW.balance IS DISTINCT FROM OLD.balance THEN")
