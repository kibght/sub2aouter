<template>
  <div class="auth-shell relative flex min-h-screen items-center justify-center overflow-hidden p-4">
    ...
    <div class="auth-decorations pointer-events-none absolute inset-0 overflow-hidden">
      <!-- Gradient Orbs -->
      <div
        class="auth-orb auth-orb-top absolute -right-40 -top-40 h-80 w-80 rounded-full bg-primary-400/20 blur-3xl"
      ></div>
      <div
        class="auth-orb auth-orb-bottom absolute -bottom-40 -left-40 h-80 w-80 rounded-full bg-primary-500/15 blur-3xl"
      ></div>
      <div
    ...
    <div class="auth-container relative z-10 w-full max-w-md">
      ...
      <div class="auth-brand mb-8 text-center">
        ...
        <template v-if="settingsLoaded">
          ...
          <h1 class="auth-site-name text-gradient mb-2 text-3xl font-bold">
            {{ siteName }}
          </h1>
          <p class="auth-site-subtitle text-sm text-gray-500 dark:text-dark-400">
            {{ siteSubtitle }}
          </p>
        </template>
      </div>
