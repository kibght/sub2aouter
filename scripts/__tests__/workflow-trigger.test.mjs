...
test('upstream theme workflow runs the full regression suite before publishing latest', async () => {
  ...
  const verifiedBundle = workflow.indexOf('name: Create immutable verified release bundle')
  const immutablePush = workflow.indexOf('name: Publish immutable image and generated release branch')
  ...
  assert.ok(backend < verifiedBundle)
  assert.ok(verifiedBundle < immutablePush)
  assert.ok(immutablePush < binaryJob)
